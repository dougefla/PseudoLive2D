...
class LivePortraitPipeline(object):
    ...
    def make_motion_template(self, I_lst, c_eyes_lst, c_lip_lst, **kwargs):
        ...
        for i in track(range(n_frames), description='Making motion templates...', total=n_frames):
            ...
            # item_dct['scale'] = 1.0 if item_dct['scale'] < 1.0 else item_dct['scale']
        ...
        template_dct = self.make_safe_motion_template(template_dct, 0.15)
        return template_dct

    def is_np_array_too_different(self, arr1, arr2, threshold=0.1):
    ...
    def make_safe_motion_template(self, template_dct, threshold=0.1):
        ...
        n_frames = safe_template_dct['n_frames']
        sum_scale = sum_exp = sum_t = sum_R = sum_kp = 0
        for i in range(n_frames):
            sum_scale += safe_template_dct['motion'][i]['scale']
            sum_exp += safe_template_dct['motion'][i]['exp']
            sum_t += safe_template_dct['motion'][i]['t']
            sum_R += safe_template_dct['motion'][i]['R']
            sum_kp += safe_template_dct['motion'][i]['kp']
        avg_scale = sum_scale / n_frames
        avg_exp = sum_exp / n_frames
        avg_t = sum_t / n_frames
        avg_R = sum_R / n_frames
        avg_kp = sum_kp / n_frames
        # Check the parameters of each frame
        # If the parameters of a frame are too different from the average parameters, replace them with the average parameters
        for i in range(n_frames):
            if self.is_np_array_too_different(safe_template_dct['motion'][i]['scale'], avg_scale, threshold):
                safe_template_dct['motion'][i]['scale'] = avg_scale
            if self.is_np_array_too_different(safe_template_dct['motion'][i]['exp'], avg_exp, threshold):
                safe_template_dct['motion'][i]['exp'] = avg_exp
            if self.is_np_array_too_different(safe_template_dct['motion'][i]['t'], avg_t, threshold):
                safe_template_dct['motion'][i]['t'] = avg_t
            if self.is_np_array_too_different(safe_template_dct['motion'][i]['R'], avg_R, threshold):
                safe_template_dct['motion'][i]['R'] = avg_R
            if self.is_np_array_too_different(safe_template_dct['motion'][i]['kp'], avg_kp, threshold):
                safe_template_dct['motion'][i]['kp'] = avg_kp
        return safe_template_dct
